...
export async function getAllPatientsAction(
  ...
  throw new Error((res as any).error || 'Failed to fetch patients via API');
}
...
export async function getPatientAction(patientId: string): Promise<Patient> {
  ...
  throw new Error((res as any).error || 'Failed to fetch patient details');
}
...
export async function editPatientAction(
  ...
  if (!res.ok) {
    return {
      status: 'error',
      message: (res as any).error || 'Failed to update patient.',
    };
  }
...
export async function deletePatientAction(patientId: number): Promise<boolean> {
  ...
  throw new Error((res as any).error || 'Failed to delete patient via API');
}
...
export async function createPatientAction(
  ...
  if (!res.ok) {
    return {
      status: 'error',
      message: (res as any).error || 'Failed to create patient.',
    };
  }
