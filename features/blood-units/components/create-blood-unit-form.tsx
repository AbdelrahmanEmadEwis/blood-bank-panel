...
import { createBloodUnitAction } from '@/features/blood-units/actions/blood-units';
import { type CreateBloodUnitValues } from '../types';
...
export function CreateBloodUnitForm() {
  const [state, action] = useActionState<ActionState<CreateBloodUnitValues>, FormData>(
  ...
  return (
    <form action={action} className='space-y-6 w-full bg-white p-6 rounded-xl border'>
      ...
      <div className='pt-4'>
        <SubmitButton pendingLabel='Creating unit...' className='w-full'>
          Create Blood Unit
        </SubmitButton>
      </div>
    </form>
